...
async def join_event(event_id: str, user_id: str = Query(...)):
...
async def submit_tip(data: dict):
...
async def street_score(ward: str):
...
async def city_leaderboard(limit: int = Query(20, le=100)):
    return await database.get_city_leaderboard(limit)

@karma_router.get("/leaderboard/ward/{ward}")
async def ward_leaderboard(ward: str):
